...
const new_event = async (data_token, data_event) => {
    ...
    if (date < dateNow)
        throw new Error('INVALID_CREDENTIALS');
    const event = await Model_1.default.findOne({ date: data_event.date });
    if (event)
    ...
    try {
        ...
        await new Model_1.default(data_event).save();
        return {
            succes: true,
...
const list_active_events = async (data_token, page_params) => {
    const user_token = data_token.user;
    ...
    try {
        let events = await Model_1.default.paginate({ is_active: true }, options);
        if (page > events.totalPages) {
            options.page = 1;
            events = await Model_1.default.paginate({ is_active: true }, options);
        }
...
const list_inactive_events = async (data_token, page_params) => {
    const user_token = data_token.user;
    let page = page_params ? parseFloat(page_params) : 1;
    const pageSize = 1;
    const options = {
    ...
    if (user_token.is_active === false)
        throw new Error('DELETED');
    try {
        let events = await Model_1.default.paginate({ is_active: false }, options);
        if (page > events.totalPages) {
            options.page = 1;
            events = await Model_1.default.paginate({ is_active: false }, options);
        }
        if (events.docs.length > 0) {
            return {
                success: true,
...
const delete_event = async (data_token, id_event) => {
    const user_token = data_token.user;
    if (user_token === undefined)
        throw new Error('INVALID_CREDENTIALS');
    if (user_token.is_active === false)
        throw new Error('DELETED');
    const event = await Model_1.default.findOne({ _id: id_event });
...
const add_participant_event = async (data_token, id_event) => {
...
const remove_participant_event = async (data_token, id_event) => {
    const user_token = data_token.user;
    if (user_token === undefined)
        throw new Error('INVALID_CREDENTIALS');
    if (user_token.is_active === false)
